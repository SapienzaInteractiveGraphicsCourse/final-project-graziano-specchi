...
let floor_Id;
let difficulty_mode = "IMPOSSIBLE";
let light_mode = "DAY";
let sec = 0;
let penalty = 0;
let escaped = 0;
...
function init() {
    ...
    if(night_mode){
		...
		scene.fog = new THREE.Fog( 0x2a2a35, 0, 75 );
		...
		scene.fog = new THREE.Fog( 0xFFFFFF, 0, 75 );
		light_mode = "DAY";
...
function animateStick( stick ){
	let tween_stick = new TWEEN.Tween(stick.rotation).to({x: stick.rotation.x-Math.PI/3, y: stick.rotation.y, z:  stick.rotation.z-Math.PI/4}, 100);
...
function checkCameraPosition(){// REALIZZARLA CON IL TIMEOUT
	...
	// CHECK FENCE WITH FIRE FOR IMPOSSIBLE
...
function clickListener(event) {
	...
	if(!first_time_clicked){
		...
		if(difficulty_mode=="EASY"){
			...
			intervalID = setInterval(function(){spawnFox()}, 2500);
		}
